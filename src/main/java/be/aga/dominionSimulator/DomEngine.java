...
//		myGui = new DomGui( this );
		myGui = new HeadlessDom(this);
		myGui.setVisible(true);
    }
    
    private void createSimpleCardStrategiesBots() {
        final DomPlayer bigMoneyCard = getBot( "Big Money Ultimate" );
        for (DomCardName theCard : DomCardName.getSafeValues()) {
           if (theCard.hasCardType( DomCardType.Kingdom )
             ...
             DomPlayer theBot = bigMoneyCard.getCopy( theCard.toString());
    ...
    public void loadSystemBots() {
		try {
			...
			showMessageDialog( "You'll need to download Java 1.8 at www.java.com to runSimulation this program!!!");
		}
		Collections.sort( bots );
	}
    
    public void showMessageDialog(Object message) throws HeadlessException {
        if (myGui instanceof DomGui) {
            JOptionPane.showMessageDialog((DomGui) myGui, message);
        }
    }

    public void showMessageDialog(Object message, String title, int messageType) throws HeadlessException {
        if (myGui instanceof DomGui) {
            JOptionPane.showMessageDialog((DomGui) myGui, message, title, messageType);
        }
    }
    ...
    public DomPlayer loadUserBotsFromXML(InputSource anXMLSource) {
		try {
			...
			e.printStackTrace();
			showMessageDialog( "Bot creation failed! Make sure you have a valid XML in your clipboard", "", JOptionPane.ERROR_MESSAGE);
		}
		return null;
	}
...
//        theBuilder.append(string).append("<BR>").append(myLog);
	...
	public void saveUserBots() {
		...
		if (showSaveDialog(fileChooser) == JFileChooser.APPROVE_OPTION) {
			Writer output = null;
			try {
    ...
    private int showSaveDialog(JFileChooser fileChooser) {
        return myGui instanceof DomGui ? fileChooser.showSaveDialog((DomGui) myGui) : 0;
    }
	...
	public void loadUserBots() {
		...
		fileChooser.setFileFilter(getFileFilter());
		fileChooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
		if (showOpenDialog(fileChooser) == JFileChooser.APPROVE_OPTION) {
			Reader input = null;
			try {
    ...
    private int showOpenDialog(JFileChooser fileChooser) {
        return myGui instanceof DomGui ? fileChooser.showOpenDialog((DomGui) myGui) : 0;
    }
	...
	public DomGui getGui() {
		return myGui instanceof DomGui ? (DomGui) myGui : null;
	}
